...
{
    REQUIRE(conf::fmt::format("{}", std::chrono::nanoseconds{45}) == "45ns");
    auto const microseconds = conf::fmt::format("{}", std::chrono::microseconds{378});
    ...
    REQUIRE(conf::fmt::format("{}", std::chrono::milliseconds{2}) == "2ms");
    REQUIRE(conf::fmt::format("{}", std::chrono::seconds{58}) == "58s");
    REQUIRE(conf::fmt::format("{}", std::chrono::minutes{42}) == "42min");
    REQUIRE(conf::fmt::format("{}", std::chrono::hours{5}) == "5h");
    REQUIRE(conf::fmt::format("{}", std::chrono::days{90}) == "90d");
}
...
TEST_CASE("Verify std::format specialisation for std::chrono::date", "[morpheus.conversion.chrono.date.formatter]")
{
    ...
    REQUIRE(conf::fmt::format("{}", day{8}) == "08");
    REQUIRE(conf::fmt::format("{}", month{10}) == "Oct");
    REQUIRE(conf::fmt::format("{}", year{1980}) == "1980");
    REQUIRE(conf::fmt::format("{}", weekday{3}) == "Wed");
    REQUIRE(conf::fmt::format("{}", weekday_indexed{Wednesday[2]}) == "Wed[2]");
    REQUIRE(conf::fmt::format("{}", weekday_last{Monday[last]}) == "Mon[last]");
    REQUIRE(conf::fmt::format("{}", month_day{month{10}, day{8}}) == "Oct/08");
    REQUIRE(conf::fmt::format("{}", month_day_last{month{10}}) == "Oct/last");
    REQUIRE(conf::fmt::format("{}", year_month{year{1980}, month{10}}) == "1980/Oct");
    REQUIRE(conf::fmt::format("{}", year_month_day{year{1980}, month{10}, day{8}}) == "1980-10-08");
    REQUIRE(conf::fmt::format("{}", year_month_day_last{year{1980}, month_day_last{month{10}}}) == "1980/Oct/last");
    REQUIRE(conf::fmt::format("{}", year_month_weekday{year{1980}, month{10}, weekday_indexed{Wednesday[2]}}) == "1980/Oct/Wed[2]");
...
TEST_CASE("Verify toString handling for std::chrono::time_zone types", "[morpheus.conversion.chrono.time_zone.toString]")
{
    ...
    REQUIRE(toString(*get_tzdb().locate_zone("America/New_York")) == "America/New_York");
    REQUIRE(toString(*get_tzdb().locate_zone("Asia/Hong_Kong")) == "Asia/Hong_Kong");
...
TEST_CASE("Verify fromString handling for std::chrono::time_zone types", "[morpheus.conversion.chrono.time_zone.fromString]")
{
    ...
    REQUIRE(fromString<conf::date::time_zone>("Asia/Hong_Kong") == *conf::date::get_tzdb().locate_zone("Asia/Hong_Kong"));
    REQUIRE(fromString<conf::date::time_zone>("Europe/London") == *conf::date::get_tzdb().locate_zone("Europe/London"));
    REQUIRE(!fromString<conf::date::time_zone>("Invalid").has_value());
}
