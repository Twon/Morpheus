...
TEMPLATE_TEST_CASE("Verify associative containers concepts", "[morpheus.containers.concepts.associative]", bool, std::int8_t, std::uint8_t, std::int16_t, std::uint16_t, std::int32_t, std::uint32_t, std::int64_t, std::uint64_t, float, double)
{
    ...
    {
        STATIC_REQUIRE(Associative<archtypes::Associative<false, false>>);
        STATIC_REQUIRE(Associative<archtypes::Associative<true, false>>);
        STATIC_REQUIRE(Associative<archtypes::Associative<false, true>>);
        STATIC_REQUIRE(Associative<archtypes::Associative<true, true>>);
    }
