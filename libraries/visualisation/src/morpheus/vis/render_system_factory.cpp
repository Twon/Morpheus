...
{
    const auto renderSystems = [](){
        std::vector<std::string> results;
        boost::hana::for_each(availableAPIs(), [&](const auto& pair)
            {
    ...
    auto nextButton = ftxui::Button("Next", [&] { current = Pages::SelectRenderSystem; screen.Exit(); });
