...
int main(int argc, char* argv[])
{
    ...
    auto const doRead = boost::hana::fix([](auto self, std::shared_ptr<boost::asio::ip::tcp::socket> socket) -> void
    {
        ...
        socket->async_read_some(boost::asio::buffer(data->data(), data->size()),
            [self, socket, data](boost::system::error_code ec, std::size_t n) mutable -> void
            {
                if (!ec)
                {
                    boost::asio::async_write(*socket, boost::asio::buffer(data->data(), n), // Write only the 'n' bytes read in.
    ...
    auto const doAccept = boost::hana::fix([&acceptor, &doRead](auto self) -> void
    {
